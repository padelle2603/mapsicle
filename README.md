...
The style is the OpenFreeMap Liberty one: free, no API key, and lighter than
the MapTiler Streets style it stands in for, 111 layers and 43 KB against 160
layers and 167 KB, with the same colours.
...
40 unit tests covering geoprojection and distance math, route parsing, locale
resolution, search term localization, style localization, the places layers and
their categories, the Google Maps link and the update version comparison.
...
The build fails if that number would not be higher than the previous release's,
because Android rejects a non-increasing `versionCode` as a downgrade and a
published release cannot be taken back.
